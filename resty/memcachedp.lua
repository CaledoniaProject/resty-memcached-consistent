...
function str2num (str)
    local n=0
    for _,c in ipairs{str:byte(1, -1)} do n=n+c end return n
end

function set_sock(self, key)

    local _hashes = self.hashes
    local _servers = self.servers

    ---
    --- Binary search algo
    ---
    local function bsearch (st, ed, val, bucket)
        local mid = ed
        while st < ed - 1 do
            mid = math.floor( (st + ed) / 2 )

            if bucket[mid] < val then
                st = mid + 1
            else
                ed = mid - 1
            end
    ...
    local function pick_server (start_id, cnt)
        local idx
        for idx = start_id, cnt + 1, 1 do
            local no = idx
            if idx > cnt then no = 1 end

            if 0 == serv.socket then
                serv.socket = tcp()
                serv.socket:settimeout (2500)
                local ok, err = serv.socket:connect (serv.host, serv.port)

                if not ok then
                    ngx.log (ngx.ERR, "Couldn't connect memcache: " .. err)
                else
--                    ngx.log (ngx.ERR, "Server " .. serv.port .. " is connected")
                    return serv.socket
                end
            else
--                    ngx.log (ngx.ERR, "Server " .. serv.port .. " is returned")
                return serv.socket
            end
    ...
    if #_hashes == 1 then
        local hash = _hashes [1]
        local serv = _servers [hash]

--        ngx.say (ngx.ERR, "hash: " .. hash .. " server: " .. serv.host)

        if 0 == serv.socket then
            serv.socket = tcp()
            serv.socket:settimeout (10000)
--            ngx.log (ngx.ERR, "Reinitialize socket: " .. serv.host)
            serv.socket:connect (serv.host, serv.port)
        end
            
        sock = serv.socket
    else
        local start_id = bsearch (1, #_hashes, str2num(key), _hashes)
        sock = pick_server (start_id, #_hashes)
    end

if sock == nil then
    ngx.log (ngx.ERR, "no suitable socket, wtf")
end

    self.sock = sock
end

function new(self, hostlist, opts)

    local _hashes = {}
    local _servers = {}

    for i in pairs(hostlist) do
        local item = hostlist[i]
